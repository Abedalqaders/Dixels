...
public class OperatingWindowTests
{
    ...
    {
        Should.Throw<ArgumentException>(() => new OperatingWindow(new TimeOnly(7, 0), new TimeOnly(7, 0)));
    }
    ...
    {
        var window = new OperatingWindow(new TimeOnly(7, 0), new TimeOnly(20, 0));
    ...
    private static OperatingWindow ToWindow(OperatingWindowSpec spec)
        => spec.IsOpen24Hours
            ...
            : new OperatingWindow(TimeOnly.Parse(spec.Open), TimeOnly.Parse(spec.Close));
}
